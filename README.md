...
- **Grafana statistics** (optional, can be toggled on/off in the dashboard): Prometheus exporter → Prometheus (400 days retention) → provisioned Grafana dashboard (temperatures, power, COP, Smart Grid, daily energy) — the exporter (`:9101/metrics`) also exposes the compressor-level data (fg=60) that is missing from Hoval's official Modbus list: lifetime energy counters (heating/cooling/DHW), running hours per mode, heat-pump flow/return, circulation pump speed, volume flow, WEZ status, error code and the per-mode output setpoints
