...
- **Home Assistant**: auto-generated Modbus package (`hoxpi.yaml` download) + optional MQTT with auto-discovery; includes a **stale-data alarm** (`binary_sensor.hoxpi_daten_veraltet`, device_class *problem*): turns ON when the bridge has not answered or `can0` has received nothing for `stale_min` minutes (default 10, configurable in `hoxpi-features.json` → `mqtt_ha.stale_min`), attributes show age and reason; plus `binary_sensor.hoxpi_kuehl_wartet` ("cooling requested, refrigeration controller waiting for circuit temperature": HC1 = 22 ∧ UKA valve open ∧ WEZ off, attributes phase 0–3 / heat-pump flow / start mark 19.5 °C) — the exporter mirrors it as `hoval_kuehl_phase` (0 none, 1 waiting, 2 start mark reached, 3 active)
