...
- **Grafana statistics** (optional, can be toggled on/off in the dashboard): Prometheus exporter → Prometheus (400 days retention) → provisioned Grafana dashboard (22 panels: temperatures, power, COP, Smart Grid, daily energy/heat per mode, compressor modulation vs. output setpoint, heat-pump flow/return/volume flow, refrigerant circuit (air inlet, suction gas, hot gas, superheat, low pressure), WEZ status timeline, cooling phase timeline (requested / waiting for circuit temperature / start mark reached / active), active fault as plain text W/B/E:nn plus fault minutes per code, running hours and compressor starts per day, ventilation) — the exporter (`:9101/metrics`) also exposes the compressor-level data (fg=60) that is missing from Hoval's official Modbus list: lifetime energy counters (heating/cooling/DHW), running hours per mode, heat-pump flow/return, circulation pump speed, volume flow, WEZ status, error code and the per-mode output setpoints
...
- **Grafana-Statistik** (optional, im Dashboard ein-/ausschaltbar): Exporter → Prometheus (400 Tage) → fertiges Grafana-Dashboard (22 Panels: Temperaturen, Leistung, COP, Smart Grid, Tagesenergie/-wärme je Betriebsart, Verdichter-Modulation vs. Leistungs-Sollwert, WP-Vorlauf/Rücklauf/Volumenstrom, Kältekreis (Lufteintritt, Sauggas, Heißgas, Überhitzung, Nieder- und Hochdruck, Verdichter-Istdrehzahl), WEZ-Status-Zeitleiste, Kühl-Phasen-Zeitleiste (angefordert / wartet auf Kreis-Temperatur / Startmarke erreicht / aktiv), aktiver Fehler im Klartext W/B/E:nn plus Störminuten je Code, Betriebsstunden und Verdichterstarts pro Tag, Lüftung) — der Exporter (`:9101/metrics`) liefert zusätzlich die Verdichter-Ebene (fg=60), die in Hovals offizieller Modbus-Liste fehlt: Lebensdauer-Energiezähler (Heizen/Kühlen/WW), Betriebsstunden je Betriebsart, WP-Vorlauf/-Rücklauf, Pumpendrehzahl, Volumenstrom, WEZ-Status, Fehlercode und die Leistungs-Sollwerte je Betriebsart
