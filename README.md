...
- **Home Assistant**: auto-generated Modbus package (`hoxpi.yaml` download) + optional MQTT with auto-discovery; includes a **stale-data alarm** (`binary_sensor.hoxpi_daten_veraltet`, device_class *problem*): turns ON when the bridge has not answered or `can0` has received nothing for `stale_min` minutes (default 10, configurable in `hoxpi-features.json` → `mqtt_ha.stale_min`), attributes show age and reason; plus `binary_sensor.hoxpi_kuehl_wartet` ("cooling requested, refrigeration controller waiting for circuit temperature": HC1 = 22 ∧ UKA valve open ∧ WEZ off, attributes phase 0–3 / heat-pump flow / return / start rule) — the exporter mirrors it as `hoval_kuehl_phase` (0 none, 1 waiting, 2 condition met, 3 active). The start rule follows the refrigeration controller's own cooling logic: it releases the compressor once the **return ≥ 19.0 °C and the flow ≥ 18.0 °C have held for 15 min without interruption** (cooling setpoint clamped to 18.0 °C + 1 K band; the dashboard shows the elapsed timer from Prometheus)
- **Grafana statistics** (optional, can be toggled on/off in the dashboard): Prometheus exporter → Prometheus (400 days retention) → provisioned Grafana dashboard (27 panels: temperatures, power, COP, Smart Grid, daily energy/heat per mode, compressor modulation vs. output setpoint, heat-pump flow/return/volume flow, refrigerant circuit (air inlet, suction gas, hot gas, superheat, low pressure), WEZ status timeline, cooling phase timeline (requested / waiting for circuit temperature / start mark reached / active), active fault as plain text W/B/E:nn plus fault minutes per code, running hours and compressor starts per day, ventilation, bridge health: warm-cache stale/loaded words, CAN registers seen, bridge uptime) — the exporter (`:9101/metrics`) also exposes the compressor-level data (fg=60) that is missing from Hoval's official Modbus list: lifetime energy counters (heating/cooling/DHW), running hours per mode, heat-pump flow/return, circulation pump speed, volume flow, WEZ status, error code and the per-mode output setpoints
- **SG-Ready / PV surplus**: full support for Hoval's Smart Grid offset registers (Use Case 8 of the Hoval Modbus guideline)
- **Fault watcher & short-cycle alarm** (fault codes decoded to plain text `W/B/E:nn` = warning/blocking/lock-out from register 1534) with **in-dashboard configuration** (Security page: thresholds for starts/h, DHW drop, webhook URL, MQTT) - no file editing needed
- **Self-healing after power cuts**: a setpoint-state guard (`watch/hoval_sollzustand.py`, cron */5) restores the basic operating mode by itself when the controller comes back in a different state (e.g. after a power failure or a service visit) — registers/values in `hoxpi-sollzustand.json`, writes only through the bridge whitelist, pause with `touch /home/admin/WARTUNG`; status on the Security page
...
- **Warm-Cache** (Neustart-Schutz): die Bridge sichert alle je vom CAN gelesenen Registerwerte minütlich und beim Stoppen nach `cache_last.json` und lädt sie beim Start als Vorbelegung (max. 48 h alt). Loxone/HA sehen nach einem Neustart der Bridge — auch bei stromloser Wärmepumpe — die letzten echten Werte statt 0 (ohne Warm-Cache lieferte z. B. 1477 Außentemperatur stundenlang 0,0 °C und löste in Loxone den Frostschutz aus). Geladene Werte gelten intern weiter als „kalt“: Schreiben ist erst erlaubt, wenn das Register vom CAN bestätigt wurde; der Fehlercode 1534 wird bewusst nicht gecacht (0 = „kein Wert“ bleibt sichtbar). Sichtbar gemacht: die Bridge schreibt alle 15 s `bridge_state.json` (geladene/noch unbestätigte Wörter), das Dashboard zeigt auf Start- und Live-Seite ein Banner, solange Werte noch aus dem Cache stammen, und der Exporter liefert `hoxpi_cache_stale`, `hoxpi_cache_loaded`, `hoxpi_can_registers_seen`, `hoxpi_bridge_uptime_seconds` (im Test 05.09.2026: 500 Wörter geladen, nach 45 s alle vom CAN bestätigt)
...
- **Home Assistant**: automatisch erzeugtes Modbus-Package (`hoxpi.yaml`-Download) + optional MQTT mit Auto-Discovery; inklusive **Stale-Alarm** (`binary_sensor.hoxpi_daten_veraltet`, device_class *problem*): geht auf EIN, wenn die Bridge seit `stale_min` Minuten nicht antwortet oder `can0` nichts mehr empfängt (Standard 10, einstellbar in `hoxpi-features.json` → `mqtt_ha.stale_min`); Attribute zeigen Alter und Grund; dazu `binary_sensor.hoxpi_kuehl_wartet` („Kühlen angefordert, Kältekreisregler wartet auf Kreis-Temperatur“: HK1 = 22 ∧ UKA-Ventil offen ∧ WEZ aus, Attribute Phase 0–3 / WP-Vorlauf / Rücklauf / Startregel) — der Exporter spiegelt das als `hoval_kuehl_phase` (0 keine, 1 wartet, 2 Bedingung erfüllt, 3 aktiv). Die Startregel folgt der Kühllogik des Kältekreisreglers: Freigabe, sobald **Rücklauf ≥ 19,0 °C und Vorlauf ≥ 18,0 °C 15 min ununterbrochen** gelten (Kühl-Sollwert auf 18,0 °C geklemmt + 1 K Band; das Dashboard zeigt den laufenden Timer aus Prometheus)
- **Grafana-Statistik** (optional, im Dashboard ein-/ausschaltbar): Exporter → Prometheus (400 Tage) → fertiges Grafana-Dashboard (27 Panels: Temperaturen, Leistung, COP, Smart Grid, Tagesenergie/-wärme je Betriebsart, Verdichter-Modulation vs. Leistungs-Sollwert, WP-Vorlauf/Rücklauf/Volumenstrom, Kältekreis (Lufteintritt, Sauggas, Heißgas, Überhitzung, Nieder- und Hochdruck, Verdichter-Istdrehzahl), WEZ-Status-Zeitleiste, Kühl-Phasen-Zeitleiste (angefordert / wartet auf Kreis-Temperatur / Startmarke erreicht / aktiv), aktiver Fehler im Klartext W/B/E:nn plus Störminuten je Code, Betriebsstunden und Verdichterstarts pro Tag, Lüftung, Bridge-Gesundheit: Warm-Cache stale/geladen, gesehene CAN-Register, Bridge-Laufzeit) — der Exporter (`:9101/metrics`) liefert zusätzlich die Verdichter-Ebene (fg=60), die in Hovals offizieller Modbus-Liste fehlt: Lebensdauer-Energiezähler (Heizen/Kühlen/WW), Betriebsstunden je Betriebsart, WP-Vorlauf/-Rücklauf, Pumpendrehzahl, Volumenstrom, WEZ-Status, Fehlercode und die Leistungs-Sollwerte je Betriebsart
...
- **Störungs-Wächter & Verdichter-Kurzzyklus-Alarm**: meldet Fehlercodes im Klartext (`W/B/E:nn` = Warnung/Blockierung/Verriegelung, dekodiert aus Register 1534), Hoch-/Niederdruck, Warmwasser-Ausfall und Kurztakten sofort an Home Assistant, Logdatei und optional Webhook; Schwellen und Webhook direkt im Dashboard einstellbar (Sicherheit → Störungs-Wächter → „Schwellen & Webhook“)
- **Selbstheilung nach Stromausfall**: ein Sollzustands-Wächter (`watch/hoval_sollzustand.py`, Cron */5) stellt die Grund-Betriebsart selbst wieder her, wenn der Regler in einem anderen Zustand zurückkommt (z. B. nach Stromausfall oder Servicebesuch) — Register/Werte in `hoxpi-sollzustand.json`, Schreiben nur über die Bridge-Whitelist, Pause mit `touch /home/admin/WARTUNG`; Status auf der Seite Sicherheit
